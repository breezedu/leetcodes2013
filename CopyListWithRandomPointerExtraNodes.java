...
public class CopyListWithRandomPointerExtraNodes {
	...
	private static RandomListNode copyRandomList(RandomListNode head) {
		...
		RandomListNode reserveP = new RandomListNode(0);// this list will store all original Nodes' next pointer;
		retP = reserveP;
		
		while(p.next!=null){
			
			//retP.random = p.next;
			retP.next = new RandomListNode(10);
			retP.next.random = p.next;
			
			retP = retP.next;
			p = p.next;
		} // end while P!=null loop; reserveP.random store all head's next pointers 
		reserveP = reserveP.next;
		
		System.out.println("\n Printout the reserveP list:");
		printRandomList(reserveP);
		...
		// now let the original listNodes' next point to their original nodes;
		RandomListNode p2 = reserveP;
		p = head;
		while(p2 != null){
			p.next = p2.random;
		...
		System.out.println("L134: printout the original head list again:");
		printRandomList(head);
	...
	private static RandomListNode creatRandomList(int num) {
		...
		for(int i=0; i<num; i++){
			...
			arrayRandom[i] = (int)(Math.random() * num*1.5); // this is the index of the node's random pointer pointing to;
