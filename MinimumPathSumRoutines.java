...
/**********************
 * For Pairs object see Pairs.java, 
 * Pairs dataset are used to storage the Row and Col indices of a node;
...
public class MinimumPathSumRoutines {
	...
	public static void main(String[] args){
		...
		trackMinPath(backTrackMatrix, mapMatrix, Row-1, Col-1, routinePairs);
	...
	private static void trackMinPath(char[][] backTrackMatrix, int[][] mapMatrix, int row, int col, ArrayList<Pairs> routinePairs) {
		...
		if(row==0 && col==0){
			 ...
			 */
			trackMinPath(backTrackMatrix, mapMatrix, row, col-1, routineLeft);
			
			trackMinPath(backTrackMatrix, mapMatrix, row-1, col, routineUp);
			...
			trackMinPath(backTrackMatrix, mapMatrix, row, col-1, routinePairs);
			...
			trackMinPath(backTrackMatrix, mapMatrix, row-1, col, routinePairs);
	...
	/********
	 * printout the routine ArrayList backwards;
	 * @param routinePairs
	 */
	private static void printRoutine(ArrayList<Pairs> routinePairs) {
		// TODO Printout all elements in the ArrayList<Pairs>, backwards:)
		int Num = routinePairs.size(); // total number of Pairs in the arrayList;
		
		for(int i=1; i<Num-1; i++){
			
			Pairs P = routinePairs.get(Num-i);
			System.out.print("[" + P.Row + "," + P.Col +"]->");
		} // end for loop;
		System.out.print("[" + routinePairs.get(0).Row +"," + routinePairs.get(0).Col +"].");
